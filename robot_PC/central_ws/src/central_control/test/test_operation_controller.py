...
def test_left_bad_right_good_runs_help():
    ...
    assert controller.state == OperationState.BUILDING_REPLACEMENT_PLAN
    assert controller.build_replacement_plan()
    assert controller.replacement_plan == ("left",)
    assert controller.current_side == "left"
    assert controller.prepare_current_side()
    assert controller.state == OperationState.SET_ARUCO
    assert controller.begin_set_aruco() == "left"
    assert controller.state == OperationState.WAIT_SET_ARUCO_DONE
    assert controller.apply_set_aruco_result(True)
    assert controller.state == OperationState.START_PINKY
    assert controller.mark_pinky_started()
    assert controller.begin_opposite_help() == "right"
    assert controller.state == OperationState.HELPING_OPPOSITE_ARM


def test_left_bad_right_bad_builds_left_first_plan():
    controller = OperationController()
    controller.apply_detection_result(LEFT_BAD_RIGHT_BAD)
    assert controller.build_replacement_plan()
    assert controller.replacement_plan == ("left", "right")
    assert controller.current_side == "left"
    assert controller.replacement_index == 0


def test_left_good_right_bad_targets_right_instead_of_completing():
    controller = OperationController()
    controller.apply_detection_result(LEFT_GOOD_RIGHT_BAD)
    assert controller.build_replacement_plan()
    assert controller.replacement_plan == ("right",)
    assert controller.current_side == "right"
    assert controller.state == OperationState.PREPARING_SIDE
...
def test_left_remove_success_requests_pinky():
    controller = OperationController()
    controller.state = OperationState.EXECUTING_LEFT_REMOVE
    controller.busy = True
    controller.apply_left_remove_result(True)
    assert controller.state == OperationState.REQUESTING_PINKY_MOVE
    assert controller.last_successful_step == "LEFT_REMOVE_BAD_TIRE"
    assert controller.mark_pinky_move_requested()
    assert controller.state == OperationState.WAITING_PINKY_COMPLETE
    assert controller.apply_pinky_status("RUNNING")
    assert controller.apply_pinky_status("COMPLETED")
