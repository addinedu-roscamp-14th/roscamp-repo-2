...
- Left arm default: `192.168.0.63:5000`.
...
`mock_transport_operations:=true` disables the Pinky bridge and automatically accepts transport-ready while preserving the `WAITING_TRANSPORT` state transition. LEFT/RIGHT ArUco place and pick services remain real. With `mock_transport_operations:=false`, the existing `/central/transport_ready` and Pinky integration remain available. Manual STOP and vehicle-motion emergency handling for both arms is unchanged.
