...
GRASP_POSITION_COMPENSATION_MM = {
    # 슬롯 1: 실제 집기 성공 위치를 유지한다.
    1: np.array([3.0, 0.0, 0.0], dtype=np.float64),
    # 슬롯 2: 실제 집기 성공 위치를 유지한다.
    2: np.array([3.0, 0.0, 8.0], dtype=np.float64),
    # 슬롯 3: 최종 도착 높이가 약 1mm 낮아 Z를 1mm 올린다.
    3: np.array([0.0, 0.0, 8.0], dtype=np.float64),
    4: np.array([0.0, 0.0, 0.0], dtype=np.float64),
}

# 슬롯별 집기 회전 보정값 [Rx, Ry, Rz] deg
# 슬롯 3은 반복 시험에서 실제 자세가 저장 자세보다
# Rx 약 -2.2도, Ry 약 +2.3도 방향으로 기울어졌다.
# 처음에는 반대 방향으로 1도씩만 보정한다.
GRASP_ROTATION_COMPENSATION_DEG = {
    1: np.array([0.0, 0.0, 0.0], dtype=np.float64),
    2: np.array([0.0, 0.0, 0.0], dtype=np.float64),
    3: np.array([2.0, -2.0, 0.0], dtype=np.float64),
    4: np.array([0.0, 0.0, 0.0], dtype=np.float64),
}
...
def main() -> None:
    print("=" * 70)
    print("중심 재등록 자세 재현 시험 슬롯 3 회전 보정 v13")
    print("슬롯 1·2 유지 / 슬롯 3 분할 안전경로 + 집기 회전 보정")
    print("=" * 70)
    ...
    print("y: 슬롯 2만 목표 Y까지 1mm씩 미세전진")
    ...
    try:
        while True:
            ...
            cv2.putText(
                display,
                "p/x approach | g/z grasp | y slot2 fine Y",
                (10, FRAME_HEIGHT - 16),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.48,
                (0, 255, 0),
                ...
                if response.get("ok"):
                    ...
                    if final_coords is not None:
                        ...
                        if (
                            ...
                            print(
                                "슬롯 2는 실패하던 6축 자동 미세보정을 "
                                "생략합니다. 필요하면 y를 눌러 Y축만 "
                                "1mm씩 보정하세요."
                            )
