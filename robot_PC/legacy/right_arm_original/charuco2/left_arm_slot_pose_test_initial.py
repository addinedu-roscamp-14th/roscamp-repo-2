...
# 두 팔이 서로 다른 IP를 쓰면 반드시 왼팔의 실제 IP로 변경한다.
ROBOT_IP = "192.168.6.1"
ROBOT_PORT = 5000

# 왼팔 초기 설정
# - 새 Hand-eye/슬롯 등록 결과는 오른팔 파일과 분리한다.
# - 오른팔에서만 확인한 슬롯별 경로·미세보정은 사용하지 않는다.
# - 실제 자동 그리퍼 동작은 왼팔 검증이 끝날 때까지 잠근다.
USE_RIGHT_ARM_SLOT_TUNING = False
ENABLE_AUTO_GRIPPER_ACTIONS = False
...
REGISTRATION_FILE = Path(
    "/home/choiminjoon/slot_pose_registration_left/"
...
# 왼팔 초기값은 미세보정을 축소하지 않는 중립값이다.
SLOT4_GRASP_Y_FINE_GAIN = 1.0

# pymycobot send_coords mode: 0=angular, 1=linear.
# 슬롯 4의 짧은 수직 상승·하강·후퇴에는 직선 모드를 사용한다.
LINEAR_MOVE_MODE = 1

# 왼팔 내려놓기 위치 보정 초기값 [X, Y, Z] mm.
...
# 왼팔 슬롯별 집기 자세 보정 초기값 [X, Y, Z] mm.
# 접근 보정과 집기 보정은 왼팔 반복시험 후 각각 따로 갱신한다.
...
# 왼팔 슬롯별 집기 회전 보정 초기값 [Rx, Ry, Rz] deg.
...
def main() -> None:
    print("=" * 70)
    print("왼팔 슬롯 재현 초기 시험 v1")
    print("왼팔 전용 Hand-eye·등록 파일 / 보정값 전체 0")
    print("자동 그리퍼 동작 잠금:", not ENABLE_AUTO_GRIPPER_ACTIONS)
    print("=" * 70)
    ...
    print("b/d/o: 왼팔 초기값에서는 자동 그리퍼 동작 잠금")
    print("y: 오른팔 슬롯 2 전용 기능이므로 비활성화")
    ...
    try:
        while True:
            ...
            cv2.putText(
                display,
                "p/x approach | g/z grasp | b lift | d place | o open",
                (10, FRAME_HEIGHT - 16),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.48,
                (0, 255, 0),
            ...
            if key in (
                ...
                if response.get("ok"):
                    ...
                    if final_coords is not None:
                        ...
                        should_settle = (
                            (
                                not USE_RIGHT_ARM_SLOT_TUNING
                                or selected_slot not in (2, 3)
                            )
                    ...
                    if (
                        ENABLE_AUTO_GRIPPER_ACTIONS
                        and selected_slot == AUTO_PICK_TEST_SLOT
                    ):
                        print(
                            "슬롯 4 육안 확인이 끝났다면 b를 누르세요. "
                            "안전 확인 후 8초 안에 b를 한 번 더 누르면 "
                            "그리퍼를 닫고 Z축으로 30mm 상승합니다."
                        )
                else:
                    last_grasp_reference_target = None
                    last_grasp_move_slot = None
                ...
                if not ENABLE_AUTO_GRIPPER_ACTIONS:
                    print(
                        "왼팔 초기 시험에서는 자동 그리퍼 닫기·상승이 "
                        "잠겨 있습니다. 캘리브레이션과 반복 위치 검증 후 "
                        "ENABLE_AUTO_GRIPPER_ACTIONS를 True로 바꾸세요."
                    )
                    continue
                ...
                if not USE_RIGHT_ARM_SLOT_TUNING:
                    print(
                        "오른팔 슬롯 2 전용 Y 미세전진은 왼팔 "
                        "초기 시험에서 비활성화되어 있습니다."
                    )
                    continue
