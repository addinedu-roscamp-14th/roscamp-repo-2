...
def test_parked_to_wait_set_camera_confirm():
    machine = WorkStateMachine()
    machine.parking_completed()
    assert machine.state == WorkState.WAIT_SET_CAMERA_CONFIRM


def test_set_camera_success_to_wait_detection_confirm():
    machine = parked_machine()
    assert machine.start_set_camera()
    machine.apply_set_camera_result(True)
...
def test_arm_failure_goes_error():
    machine = parked_machine()
    assert machine.start_set_camera()
    machine.apply_set_camera_result(False, "left failed")
    assert machine.state == WorkState.ERROR
    assert machine.last_error == "left failed"
