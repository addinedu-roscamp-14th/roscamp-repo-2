...
python3 src/tire_vision/tire_vision/tire_marker_test.py \
  --model models/tire_best.pt --source 2
```
...
The measured defaults are `bad_max_ratio=0.28`,
`good_min_ratio=0.32`, and `max_ratio_std=0.02`. A median at or below the
...
Marker miss handling holds the last stable GOOD/BAD for 3 consecutive misses, returns RECHECK on misses 4-5, and returns ERROR from miss 6.
Marker detection uses HSV [15,40,60] through [42,255,255], minimum area 5, and a 1.15-expanded upper tire ROI. Missing marker frames do not add an invalid ratio to history. The last stable result is held for misses 1-3, misses 4-5 return RECHECK, and miss 6 onward returns ERROR.
...
`bad_tire`, and a stable GOOD ratio is returned as `good_tire`. Missing tires or masks return ERROR. Marker misses 1-3 return the cached stable detection when one exists; misses 4 and later return an empty detections list while the status progresses from RECHECK to ERROR.

`vision_service` and `vision_server_node` retain the existing TCP request and legacy result fields. Bilateral results additionally expose `marker_state` diagnostics. The existing mock detector remains available through
`mock_mode: true`.
