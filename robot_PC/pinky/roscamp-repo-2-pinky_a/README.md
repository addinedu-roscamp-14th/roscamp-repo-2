...
# TCP 명령 대기 서버(IDLE로 시작)
ros2 run pinky_goal_pid pinky_tcp_server --ros-args   --params-file install/pinky_goal_pid/share/pinky_goal_pid/config/pinky_tcp_server.yaml
```
4. 개발 중 빌드 명령어
```bash
...
pinky_tcp_server는 0.0.0.0:7000에서 UTF-8 newline JSON 요청을 받습니다.
노드와 Nav2가 준비되어도 기본 상태는 IDLE이며 START_ROUTE 전에는
NavigateToPose goal을 보내지 않습니다. 기존 nav2_waypt 실행 파일 역시 자동 시작이
제거되어 IDLE로 대기하지만, TCP 운용에는 pinky_tcp_server를 사용합니다.

상태 확인(주행하지 않음):

~~~bash
printf '%s\n' '{"version":1,"request_id":"status-1","command":"GET_STATUS"}' \
  | nc -N 127.0.0.1 7000
~~~

배포 템플릿은 deploy/systemd/에 있습니다. 실제 Raspberry Pi의 소스 경로가
/home/pinky/roscamp-repo-2-pinky_a와 다르면 설치 전에 템플릿의
WorkingDirectory, map, setup 경로를 실제 경로에 맞춰야 합니다. 기존
pinky-bringup.service는 변경하지 않습니다.

~~~bash
sudo install -m 0644 deploy/systemd/pinky-navigation.service /etc/systemd/system/
sudo install -m 0644 deploy/systemd/pinky-route.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable pinky-navigation.service pinky-route.service
sudo systemctl start pinky-navigation.service pinky-route.service
~~~

의존 순서는 pinky-bringup.service → pinky-navigation.service →
pinky-route.service입니다. bringup_robot.launch.xml만으로는 map server, AMCL,
planner/controller, BT navigator 또는 navigate_to_pose action server가 생성되지
않습니다.

nav2_params.yaml에는 기존 실측 초기 pose와 set_initial_pose: true가 이미
정의되어 있습니다. 본 TCP 변경에서는 이 값과 map을 수정하지 않았습니다.
