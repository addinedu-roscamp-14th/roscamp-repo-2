"""ROS2 + OpenCV keyboard test node for the restored left-arm ArUco workflow."""
...
from .robot_client import LeftArmRobotClient
...
class LeftArmTransportNode:
    def __init__(self, ros_node):
        ...
        self.client = LeftArmRobotClient(
            p("robot_host").value,
            p("robot_port").value,
            p("connect_timeout_sec").value,
            p("command_timeout_sec").value,
        )
        self.camera = LeftArmArucoCamera(
    ...
    def pick(
        ...
        try:
            ...
            self.log.exception(
                "left-arm transport pick blocked: %s",
                error,
            )
...
def main(args=None):
    ...
    try:
        ...
        app = LeftArmTransportNode(
            node
        )

        node.get_logger().info(
            "left_arm_transport_pick started"
        )
        node.get_logger().info(
            "camera_device="
        ...
        node.get_logger().info(
            "left_arm_transport_pick interrupted"
        )

    except Exception as error:
        node.get_logger().fatal(
            "left_arm_transport_pick failed: "
            f"{error}"
        )
        raise
