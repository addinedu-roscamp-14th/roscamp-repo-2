...
class CentralParkingStateMachine:
    ...
    def bypass_parking(self):
        ...
        self.state = CentralState.WAIT_SET_CAMERA_CONFIRM
        self.last_status = "PARKING BYPASSED"
    ...
    def process_detections(self, detections, now=None) -> ParkingFrameResult:
        ...
        if parking_state == ParkingState.PARKED and not self.parked_latched:
            ...
            parked_event = True
            self.state = CentralState.WAIT_SET_CAMERA_CONFIRM
            self.last_status = "PARKED: WAIT SPACE"
...
class ManualCentralRuntime:
    ...
    def start_current_step_worker(self):
        ...
        if state == WorkState.WAIT_SET_CAMERA_CONFIRM and self.work_state_machine.start_set_camera():
            self._submit_worker("SET_CAMERA", self._worker_set_camera)
        elif state == WorkState.WAIT_DETECTION_CONFIRM and self.work_state_machine.start_detection():
    ...
    def _apply_worker_result_if_ready(self):
        ...
        if operation == "SET_CAMERA":
            ...
            self.work_state_machine.apply_set_camera_result(success, self._result_message(result))
