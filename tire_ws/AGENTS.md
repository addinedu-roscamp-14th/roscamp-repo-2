...
이 문서는 `robot_R` 저장소를 수정하는 개발자와 AI 코딩 에이전트가 따라야 할 작업 규칙을 정의합니다.

## 1. 프로젝트 범위

현재 저장소는 무인 타이어 정비 시스템의 **오른쪽 JetCobot 로봇팔** 제어를 담당합니다.
...
명시적인 요구가 없는 한 포트 `5001`을 다른 값으로 변경하지 마십시오.
...
6. 오른팔 설정과 왼팔 설정을 한 파일에 무분별하게 혼합하지 않습니다.
...
```bash
nc -vz -w 3 <RIGHT_ARM_IP> 5001
```

명령 테스트:

```bash
printf 'HELP\n' | nc <RIGHT_ARM_IP> 5001
```
