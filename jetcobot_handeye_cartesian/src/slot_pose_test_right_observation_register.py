...
OBSERVATION_ANGLES = [
    102.48, 49.92, -73.56, -30.05, -8.08, -31.11
]
OBSERVATION_SPEED = 3
...
# 슬롯 1: z 한 번으로 coarse 이동 후 Y/Z 자동 단일축 마무리.
# 별도 키는 사용하지 않는다. 각 축은 1mm씩만 움직이며,
# 실제 이동이 명령 반대 방향이거나 거의 움직이지 않으면 즉시 중단한다.
...
GRASP_POSITION_COMPENSATION_MM = {
    ...
    2: np.array([-2.0, 0.0, 5.0], dtype=np.float64),
...
def auto_single_coord_settle_slot1(
    robot: RobotClient,
    display: np.ndarray,
    axis_id: int,
    axis_name: str,
    ...
    """
    슬롯 1 집기 coarse 이동 후 단일 좌표축만 1mm씩 자동 보정한다.
    axis_id는 pymycobot 기준 1=X, 2=Y, 3=Z.
    추가 키 입력 없이 z 처리 안에서 호출한다.
    """
    axis_index = axis_id - 1

    start_pose = robot.request("POSE")
    if not start_pose.get("ok"):
        print(
            f"슬롯 1 {axis_name} 자동보정 시작 POSE 실패:",
            start_pose,
        )
        return None, False

    baseline = np.asarray(
        start_pose["coords_mm_deg"],
        dtype=np.float64,
    )
    ...
    if requested_total > max_total_mm:
        print(
            f"슬롯 1 {axis_name} 자동보정 차단: "
            f"필요 이동량 {requested_total:.2f}mm가 "
            f"최대 {max_total_mm:.2f}mm를 초과합니다."
        )
    ...
    for step_index in range(1, max_steps + 1):
        ...
        if not pose_now.get("ok"):
            print(
                f"슬롯 1 {axis_name} 자동보정 중 POSE 실패:",
                pose_now,
            )
            return latest.tolist(), False

        current = np.asarray(
            pose_now["coords_mm_deg"],
            dtype=np.float64,
        )
        remaining = float(
            target_value - current[axis_index]
        )
        ...
        if returned is None:
            ...
            returned = final_pose.get("coords_mm_deg")

        if returned is None:
            print(
                f"슬롯 1 {axis_name} 자동보정 후 "
                "좌표를 읽지 못했습니다."
            )
            return current.tolist(), False
    ...
    final_pose = robot.request("POSE")
    final_coords = final_pose.get("coords_mm_deg")

    if final_coords is None:
        final_coords = latest.tolist()

    final_remaining = float(
        target_value - float(final_coords[axis_index])
    )
    ...
    return (
        [float(value) for value in final_coords],
        abs(final_remaining) <= tolerance_mm,
    )


def main() -> None:
    print("=" * 70)
    print("오른팔 슬롯 등록좌표 + 슬롯1 단일축보정 제거 REGISTER v3")
            ...
            if key in (
                ...
                if response.get("ok"):
                    ...
                    if final_coords is not None:
                        ...
                        # 슬롯 1은 이 자세에서 MOVE_SINGLE_COORD가
                        # Y/Z 축 결합과 큰 Z 하강을 일으킨 반복 로그가 있어
                        # 자동 단일축 미세보정을 사용하지 않는다.
                        # z 한 번의 등록좌표 MOVE_COORDS 결과를 그대로 유지한다.
                        if selected_slot == 1:
                            print(
                                "슬롯 1은 단일축 Y/Z 자동보정을 생략합니다. "
                                "등록좌표 grasp coarse 결과를 그대로 유지합니다."
                            )

                        should_settle = (
                            selected_slot not in (1, 2, 3)
